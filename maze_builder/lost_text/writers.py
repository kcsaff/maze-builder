...
from maze_builder.lost_text.base import TextGenerator
...
class WriterDef(TextGenerator):
    NEGATIVE_STATUS = WeightedChoice({  # Used as '{}' or 'I'm so {}...'
        'hungry': 2,
        'cold': 2,
        'tired': 5,
        'exhausted': 1,
    ...
    INTENSIFIED_NEGATIVE_STATUS = WeightedChoice({
        'awfully {NEGATIVE_STATUS}': 1,
        'amazingly {NEGATIVE_STATUS}': 1,
        'cursedly {NEGATIVE_STATUS}': 1,
        'critically {NEGATIVE_STATUS}': 1,
        'deathly {NEGATIVE_STATUS}': 1,
        'meagerly {NEGATIVE_STATUS}': 0.2,
        'super-{NEGATIVE_STATUS}': 1,
        'devastatingly {NEGATIVE_STATUS}': 1,
        'terribly {NEGATIVE_STATUS}': 1,
        'dreadfully {NEGATIVE_STATUS}': 1,
        'wickedly {NEGATIVE_STATUS}': 1,
        'disgracefully {NEGATIVE_STATUS}': 1,
        'completely {NEGATIVE_STATUS}': 1,
        'reprehensibly {NEGATIVE_STATUS}': 1,
        'unforgivably {NEGATIVE_STATUS}': 1,
        'unpleasantly {NEGATIVE_STATUS}': 1,
        'wretchedly {NEGATIVE_STATUS}': 1,
    })

    FULL_NEGATIVE_STATUS = WeightedChoice({
        '{INTENSIFIED_NEGATIVE_STATUS}': 1,
        '{NEGATIVE_STATUS}': 1,
    })

    NEGATIVE_SENTENCE = WeightedChoice({
        '{FULL_NEGATIVE_STATUS}.': 30,
        '{FULL_NEGATIVE_STATUS}!': 10,
        '{FULL_NEGATIVE_STATUS} again?': 10,
        '{FULL_NEGATIVE_STATUS} now.': 2,
        '{FULL_NEGATIVE_STATUS} here.': 5,
        '{FULL_NEGATIVE_STATUS} here!': 2,
        '{FULL_NEGATIVE_STATUS}. Now I know what that means.': 1,
        '{FULL_NEGATIVE_STATUS}: and not for the first time.': 1,
        '{FULL_NEGATIVE_STATUS} -- and not for the first time.': 1,
        '{FULL_NEGATIVE_STATUS}? Yes, always.': 1,
        'I feel {FULL_NEGATIVE_STATUS}.': 10,
        'I feel so {FULL_NEGATIVE_STATUS}.': 5,
        'I feel so {FULL_NEGATIVE_STATUS}!': 5,
        'I\'m {FULL_NEGATIVE_STATUS}.': 10,
        'I\'m so {FULL_NEGATIVE_STATUS}.': 5,
        'Will I always be so {FULL_NEGATIVE_STATUS}?': 1,
        'Why am I so {FULL_NEGATIVE_STATUS}?': 1,
        'No one knows how {FULL_NEGATIVE_STATUS} I am.': 1,
        'Has anyone ever been so {FULL_NEGATIVE_STATUS} before?': 1,
        'Has anyone ever been so {FULL_NEGATIVE_STATUS}?': 1,
        'Has anyone ever felt so {FULL_NEGATIVE_STATUS}?': 1,
        'I never want to feel this {FULL_NEGATIVE_STATUS} again.': 1,
        'I hope I\'ll never be so {FULL_NEGATIVE_STATUS} again.': 1,
        'I can\'t stand being so {FULL_NEGATIVE_STATUS}.': 1,
        'I\'ve never been so {FULL_NEGATIVE_STATUS}.': 1,
        'I\'ve never been so {FULL_NEGATIVE_STATUS} before.': 1,
        'Before this trip, I\'d never been so {FULL_NEGATIVE_STATUS}.': 1,
        'At home, no one is ever so {FULL_NEGATIVE_STATUS}.': 1,
        'So {FULL_NEGATIVE_STATUS} a person can be.': 1,
        'So {FULL_NEGATIVE_STATUS} a person can feel.': 1,
        'We weren\'t meant to feel so {FULL_NEGATIVE_STATUS}.': 1,
        'I never knew what it was like to be so {FULL_NEGATIVE_STATUS}.': 1,
        'No one has ever been so {FULL_NEGATIVE_STATUS}.': 1,
        'I could write a book about being so {FULL_NEGATIVE_STATUS}.': 1,
        'Even in my dreams, I\'m {FULL_NEGATIVE_STATUS}.': 1,
        'I\'m as {FULL_NEGATIVE_STATUS} as I\'ve ever been.': 1,
        'Why does God allow us to be so {FULL_NEGATIVE_STATUS}?': 1,
        'Would I have come this way, if I\'d known I\'d be so {FULL_NEGATIVE_STATUS}?': 1,
    })

    GREAT_PAIN = WeightedChoice({
        'diarrhea': 10,
        'pain': 10,
        'hurts': 10,
        'giardia': 1,
...
class LostTextWriter(object):
    ...
    def negative_status(self, text):
        return text + _fix_sentence(self.writer('NEGATIVE_SENTENCE')) + ' '

    def lost_sentence(self, text):
        sentence = LOST_SENTENCE()
